...
// Branchless materialization of an i8 boolean from an i16 UNSIGNED comparison.
// SUB_16 leaves CY = (first < second) unsigned (its expansion ends with the
// high-byte SBB, and the trailing MOV preserves CY), so `SBB A ; ANI 1` gives
// CY as 0/1 and `SBB A ; INR A` gives !CY - no diamond, no PHI, and no subreg
// COPY, so it is safe at every opt level (SUB_16 is already used at -O0).
//   ULT = a<b            UGE = a>=b (invert)
//   UGT = ULT(b,a)       ULE = UGE(b,a)  (swap operands)
MachineBasicBlock *I8085TargetLowering::insertCond16Set(MachineInstr &MI,
  ...
  bool invert = false;
  switch (Opc) {
  case I8085::SET_ULT_16: break;
  case I8085::SET_UGE_16: invert = true; break;
  case I8085::SET_UGT_16: std::swap(first, second); break;
  case I8085::SET_ULE_16: std::swap(first, second); invert = true; break;
  }

  // We only need the borrow (CY) SUB_16 leaves behind; the 16-bit difference is
  // dead.  SUB_16 has no tied dst (Constraints="") and Defs=[SREG,A].
  unsigned diff = MRI.createVirtualRegister(getRegClassFor(MVT::i16));
  BuildMI(*MBB, It, dl, TII.get(I8085::SUB_16))
      .addReg(diff, RegState::Define | RegState::Dead)
      .addReg(first)
      .addReg(second);
  BuildMI(*MBB, It, dl, TII.get(I8085::SBB)).addReg(I8085::A);
  if (invert)
...
MachineBasicBlock *I8085TargetLowering::insertEqualityCond16Set(MachineInstr &MI,
  ...
  // Branchless i16 equality (O1+ only - it extracts operand bytes via subreg
  // COPY, which the -O0 fast allocator mishandles across a loop back-edge, so
  // -O0 falls through to the diamond below).  Combine the byte differences into
  // A (0 iff equal), then `SUI 1` makes CY = (A==0) = equal, and SBB A;ANI 1 /
  // INR A materialize the 0/1 - no branches, no PHI.
  if (getTargetMachine().getOptLevel() != CodeGenOptLevel::None) {
    ...
    auto Byte = [&](unsigned Pair, unsigned SubIdx) {
      MRI.constrainRegClass(Pair, &I8085::GR16PairRegClass);
      unsigned V = MRI.createVirtualRegister(RC8);
      BuildMI(*MBB, It, dl, TII.get(TargetOpcode::COPY), V)
          .addReg(Pair, 0, SubIdx);
      return V;
    };
    unsigned aLo = Byte(first, I8085::sub_lo), aHi = Byte(first, I8085::sub_hi);
    unsigned bLo = Byte(second, I8085::sub_lo), bHi = Byte(second, I8085::sub_hi);

    BuildMI(*MBB, It, dl, TII.get(I8085::MOV))
        .addReg(I8085::A, RegState::Define).addReg(aLo);
    BuildMI(*MBB, It, dl, TII.get(I8085::XRA)).addReg(bLo);
    unsigned t = MRI.createVirtualRegister(RC8);
    BuildMI(*MBB, It, dl, TII.get(I8085::MOV))
        .addReg(t, RegState::Define).addReg(I8085::A);
    BuildMI(*MBB, It, dl, TII.get(I8085::MOV))
        .addReg(I8085::A, RegState::Define).addReg(aHi);
    BuildMI(*MBB, It, dl, TII.get(I8085::XRA)).addReg(bHi);
    BuildMI(*MBB, It, dl, TII.get(I8085::ORA)).addReg(t);   // A = 0 iff equal
    BuildMI(*MBB, It, dl, TII.get(I8085::SUI)).addImm(1);   // CY = equal
    BuildMI(*MBB, It, dl, TII.get(I8085::SBB)).addReg(I8085::A);
...
MachineBasicBlock *I8085TargetLowering::insertSignedCond16Set(MachineInstr &MI,
  ...
  // Branchless i16 SIGNED comparison (O1+ only - subreg-COPY byte extraction,
  // same -O0 caveat as equality above).  Bias the high bytes by XRI 0x80 to
  // turn the signed compare into an unsigned one, subtract low then biased-high
  // (MOV preserves CY between the SUB and SBB) so CY = signed(first<second),
  // then SBB A;ANI 1 / INR A give the 0/1.
  //   SLT = a<b            SGE = a>=b (invert)
  //   SGT = SLT(b,a)       SLE = SGE(b,a)  (swap operands)
  if (getTargetMachine().getOptLevel() != CodeGenOptLevel::None) {
    MachineRegisterInfo &MRI = MBB->getParent()->getRegInfo();
    ...
    bool invert = false;
    switch (Opc) {
    case I8085::SET_LT_16: break;
    case I8085::SET_GE_16: invert = true; break;
    case I8085::SET_GT_16: std::swap(first, second); break;
    case I8085::SET_LE_16: std::swap(first, second); invert = true; break;
    }
    const TargetRegisterClass *RC8 = getRegClassFor(MVT::i8);
    ...
    unsigned tA = Save();
    BuildMI(*MBB, It, dl, TII.get(I8085::MOV))
        .addReg(I8085::A, RegState::Define).addReg(bHi);
    BuildMI(*MBB, It, dl, TII.get(I8085::XRI)).addImm(0x80);
    unsigned tB = Save();
    // low - low, then biased-high SBB -> CY = signed(first < second)
    BuildMI(*MBB, It, dl, TII.get(I8085::MOV))
        .addReg(I8085::A, RegState::Define).addReg(aLo);
    BuildMI(*MBB, It, dl, TII.get(I8085::SUB)).addReg(bLo);
    BuildMI(*MBB, It, dl, TII.get(I8085::MOV))
        .addReg(I8085::A, RegState::Define).addReg(tA);
    BuildMI(*MBB, It, dl, TII.get(I8085::SBB)).addReg(tB);
    BuildMI(*MBB, It, dl, TII.get(I8085::SBB)).addReg(I8085::A);
    if (invert)
      BuildMI(*MBB, It, dl, TII.get(I8085::INR))
          .addReg(I8085::A, RegState::Define).addReg(I8085::A);
...
// Branchless materialization of an i8 boolean from an i32 EQUALITY comparison.
// SUB_32 computes diff = a - b, then SUBI_32 diff,1 borrows iff diff == 0, i.e.
// CY = (a == b); then SBB A;ANI 1 / INR A give the 0/1.  Memory-backed GR32
// operands, no subreg COPY -> valid at every opt level.
MachineBasicBlock *I8085TargetLowering::insertEqualityCond32Set(MachineInstr &MI,
  ...
  unsigned diff = MRI.createVirtualRegister(RC32);
  BuildMI(*MBB, It, dl, TII.get(I8085::SUB_32))
      .addReg(diff, RegState::Define).addReg(first).addReg(second);
  unsigned dec = MRI.createVirtualRegister(RC32);
  BuildMI(*MBB, It, dl, TII.get(I8085::SUBI_32))
      .addReg(dec, RegState::Define).addReg(diff).addImm(1);
  BuildMI(*MBB, It, dl, TII.get(I8085::SBB)).addReg(I8085::A);
...
// Branchless materialization of an i8 boolean from an i32 UNSIGNED comparison.
// SUB_32 leaves CY = (first < second) unsigned (its byte loop ends with a store
// that preserves CY), so SBB A;ANI 1 / INR A give the 0/1.  Memory-backed GR32
// operands, no subreg COPY -> valid at every opt level.
//   ULT = a<b   UGE = a>=b (invert)   UGT = ULT(b,a)   ULE = UGE(b,a) (swap)
MachineBasicBlock *I8085TargetLowering::insertCond32Set(MachineInstr &MI,
  ...
  bool invert = false;
  switch (Opc) {
  case I8085::SET_ULT_32: break;
  case I8085::SET_UGE_32: invert = true; break;
  case I8085::SET_UGT_32: std::swap(first, second); break;
  case I8085::SET_ULE_32: std::swap(first, second); invert = true; break;
  }

  unsigned diff = MRI.createVirtualRegister(getRegClassFor(MVT::i32));
  BuildMI(*MBB, It, dl, TII.get(I8085::SUB_32))
      .addReg(diff, RegState::Define).addReg(first).addReg(second);
  BuildMI(*MBB, It, dl, TII.get(I8085::SBB)).addReg(I8085::A);
  if (invert)
...
// Branchless materialization of an i8 boolean from an i32 SIGNED comparison.
// Bias both operands by XOR 0x80000000 (flips the sign bit -> signed compare
// becomes unsigned), then SUB_32 leaves CY = signed(first < second), and
// SBB A;ANI 1 / INR A give the 0/1.  All GR32 operands are memory-backed
// imaginary registers (no subreg COPY), so this is valid at every opt level.
//   SLT = a<b   SGE = a>=b (invert)   SGT = SLT(b,a)   SLE = SGE(b,a) (swap)
MachineBasicBlock *I8085TargetLowering::insertSignedCond32Set(MachineInstr &MI,
  ...
  bool invert = false;
  switch (Opc) {
  case I8085::SET_LT_32: break;
  case I8085::SET_GE_32: invert = true; break;
  case I8085::SET_GT_32: std::swap(first, second); break;
  case I8085::SET_LE_32: std::swap(first, second); invert = true; break;
  }
  const TargetRegisterClass *RC32 = getRegClassFor(MVT::i32);

  unsigned ba = MRI.createVirtualRegister(RC32);
  BuildMI(*MBB, It, dl, TII.get(I8085::XORI_32))
      .addReg(ba, RegState::Define).addReg(first).addImm(0x80000000LL);
  unsigned bb = MRI.createVirtualRegister(RC32);
  BuildMI(*MBB, It, dl, TII.get(I8085::XORI_32))
      .addReg(bb, RegState::Define).addReg(second).addImm(0x80000000LL);
  unsigned diff = MRI.createVirtualRegister(RC32);
  BuildMI(*MBB, It, dl, TII.get(I8085::SUB_32))
      .addReg(diff, RegState::Define).addReg(ba).addReg(bb);
  BuildMI(*MBB, It, dl, TII.get(I8085::SBB)).addReg(I8085::A);
  if (invert)
    BuildMI(*MBB, It, dl, TII.get(I8085::INR))
        .addReg(I8085::A, RegState::Define).addReg(I8085::A);
